...
## 6.5 ギフト（リビールカード）— 2026-08-18

LP のギフト欄は以前から "come with a cinematic reveal card" と書いていたが、
**実体が無かった**。ギフト購入という選択肢も無かった（`GiftOrderForm` は
管理者がインフルエンサーに無償枠を配る画面）。両方とも売っているページ上の
守られていない約束だったので、片方を作り、もう片方は文言を直した。

**カードは全注文に付く。ギフト限定の特典にしていない。** satori -> resvg で
描くだけで生成AIを使わないので原価がゼロ。原価のかからないものを出し惜しみして
LP の記述を嘘のままにする理由が無い。Stripe は一切触っていない。

**トークンが2つある。ここを混ぜてはいけない。**

| | 開くもの | 渡せるもの |
|---|---|---|
| `approveToken` | `/approve/[token]`（買った人の画面） | 全部 |
| `shareToken` | `/premiere/[token]`（贈られた人の画面） | **映画とポスターだけ** |

`/approve` にはダウンロード・評価・SNS許諾・アドオン購入が並ぶ。カードに
approveToken を刷ると、贈られた人に買った人の操作権を渡すことになる。
`/premiere` は再生とポスターとダウンロード2つだけで、他の操作要素は無い。

- カードは**保存しない。**要求ごとに描く（中身は名前・タグライン・トークンだけ）
- `kind=card` と `kind=take` は shareToken では **404**（403 にすると存在を教える）
- 未納品の注文で `/premiere` は **404** — 贈る人が事前に試して作りかけを見せない
- 本番で `APP_BASE_URL` が無ければ**描画を落とす** — localhost の QR を刷った紙が
  人手に渡る方が、ダウンロード1件の失敗より悪い
- ダウンロードの証跡に `via: "buyer" | "share-link"` を残す。贈られた人の
  ダウンロードは**買った人が受け取った証拠にはならない**ので、混ぜない
- `shareToken` は NULL 可 + 遅延生成（`lib/share-token.ts`）。移行の埋め漏れで
  機能が無言で消えるのを防ぐ

**移行SQL は適用済み**（2026-08-18、全15注文にユニークなトークン）。
スキーマに列を宣言した時点で、列の無い DB に対する全件取得が全部落ちる
= **サイト全体が落ちる**。同種の列を足すときは必ず SQL を先に流すこと。

実注文の本番 QR がスマホで開くところまで確認済み。
