...
| Stripe | ✅ **本番稼働**（2026-08-12）。Price4つ・本番キー・本番webhook・ToS URL・アカウント認証すべて確認済み。`cs_live_` セッションが両プランで作成でき、Checkout画面の金額と同意文も目視済み |
| メール送信（Resend） | ✅ DKIM (`resend._domainkey`) と SPF (`send.marqueetails.com` → amazonses) 設定済み |
| メール**受信** | ✅ Cloudflare Email Routing で `support@` を転送（2026-08-10）。**Gmail側で迷惑メール判定されたので、`to:support@marqueetails.com` を「迷惑メールにしない」フィルタが要る** |
| DMARC | ✅ `v=DMARC1; p=none; rua=mailto:support@marqueetails.com` |
| 流入計測 | ❌ **解析が1つも入っていない。** SNSの投稿は始まっているが、訪問者数も流入元も測れていない。注文数しか分からないので「オファーが弱い」と「流入が足りない」を区別できない |
...
**実測値（2026-08-12、Director's Cut 1件・LoRAコピーなし）。**
注文 `cmsovxam4…`（LALA）で残高を時刻ごとに記録したもの:

| 区間 | 残高 | 差分 |
|---|---|---|
| LoRA学習中（未課金） | 36.47940134 | — |
| **LoRA学習＋特徴抽出＋ポートレート 完了** | 26.72204804 | **−$9.76** |
| 絵コンテ生成 途中 | 26.23259784 | −$0.49 |
| 〃 | 24.87552204 | −$1.36 |
| **絵コンテ6カット完成** | 24.11217534 | −$0.76 |
| **写真提出 → 絵コンテ 合計** | | **−$12.37** |

別途、本編クリップ6本（Kling）で **$5.66**（2026-08-04、24.42734844 →
18.76340134）。**1注文の総額はおよそ $20〜25** になる。

**⚠️ LoRA学習は「約$2」ではない。** この数字は測らずに引き継がれ、
LORA-STORYBOARD-SPEC.md §2/§4、scripts/copy-lora.ts、この手順書の各所で
引用されていた。**実測は約$9.5**（上表の −$9.76 から特徴抽出とポートレートの
数十セントを引いた値）。5倍近い開きがある。

これが効くのは2箇所:
- **LoRAコピー（scripts/copy-lora.ts）の節約額は $2 ではなく約 $9.5。**
  同じペットで作り直すときは、コピーする価値が想定よりずっと大きい。
- **無償枠を配るときの予算。** 30本なら $600 ではなく **$600〜750**。

**課金は完了時にまとめて落ちる。** 生成の途中は残高が動かないので、
「減っていない＝走っていない」とは読めない（今日その読み違いを一度している）。
走っているかは Trigger.dev の Runs で見る。

総額をもう一度測るときは、1注文の開始前と納品後に上のコマンドで残高を取る。
